    ...
    try:
        with open(f"{log_id}-camera.mcap", "wb") as stream, Writer(stream) as writer:
            ...
            for cam_calib, cam_intrin in zip(calibs.to_numpy(), intrins.to_numpy()):
                ...
                if not Path.exists(dataroot / log_id / f"sensors/cameras/{id}/"):
                    e = FileNotFoundError()
                    e.filename =f"{dataroot}/{log_id}/sensors/cameras/{id}/"
                    raise e
        ...
        print(f"Failed to convert camera data to MCAP. File not found `{e.filename}`")
        os.remove(f"{log_id}-camera.mcap")
